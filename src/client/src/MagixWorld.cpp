...
void MagixWorld::initialize(SceneManager *sceneMgr, RenderWindow *window, MagixExternalDefinitions *def, MagixGameStateManager *gameStateMgr, MagixSoundManager *soundMgr, MagixCollisionManager *collisionMgr, MagixSkyManager *skyMgr)
{
	mSceneMgr = sceneMgr;
	mWindow = window;
	mDef = def;
...
void MagixWorld::addWaterPlane(const Vector3 &position, const Real &scaleX, const Real &scaleZ, const String &matName, const String &playSound)
{
	numWaterPlanes += 1;
