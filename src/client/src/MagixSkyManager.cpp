...
void MagixSkyManager::initialize(SceneManager *sceneMgr, MagixExternalDefinitions *def, MagixCamera *camera, MagixSoundManager *soundMgr)
{
	mSceneMgr = sceneMgr;
	mDef = def;
	mCamera = camera;
...
#pragma warning(disable : 4482)
	...
	if (dayTime >= 1800 && dayTime<1900)
	{
		...
		const Real tFade = (Real)((1900 - dayTime)*1.0 / 100);
		Vector4 skyFade((Real)(0.4 + tFade*0.6), (Real)(0.4 + tFade*0.6), (Real)(0.5 + tFade*0.5), 1);
		...
		static_cast<MaterialPtr>(MaterialManager::getSingleton().getByName("Sky/CloudDome")).get()->setSelfIllumination(cloudColour);
		static_cast<MaterialPtr>(MaterialManager::getSingleton().getByName("Sky/SunRadiance")).get()->setSelfIllumination((Real)(tFade*1.0), (Real)(tFade*0.7), 0);
	...
	if (dayTime >= 500 && dayTime<600)
	{
		...
		static_cast<MaterialPtr>(MaterialManager::getSingleton().getByName("Sky/CloudDome")).get()->setSelfIllumination(cloudColour);
		static_cast<MaterialPtr>(MaterialManager::getSingleton().getByName("Sky/SunRadiance")).get()->setSelfIllumination((Real)(tFade*1.0), (Real)(tFade*0.8), 0);
			...
			else
			{
				...
				else
				{
					...
					if (weatherTime <= (weatherEvent.start + 200))
					{
						...
						if (hasWeatherSound)mSoundManager->setAmbientSoundVolume((float)((weatherTime - (weatherEvent.start + 100))*0.008));
					}
					else if (weatherTime >= (weatherEvent.end - 200) && weatherTime<(weatherEvent.end - 100))
					{
						...
						if (hasWeatherSound)mSoundManager->setAmbientSoundVolume((float)(((weatherEvent.end - 100) - weatherTime)*0.008));
					}
					else if (weatherTime >= (weatherEvent.end - 100))
					{
