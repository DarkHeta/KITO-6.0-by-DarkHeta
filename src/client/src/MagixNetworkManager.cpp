...
void MagixNetworkManager::update(const FrameEvent evt)
{
	if (acceptedFlag)
	{
		...
		{
			moveUpdateCount += (Real)(mUnitManager->getPlayer()->getIsCrouching() ? 0.75 : (mUnitManager->getPlayer()->getIsWalking() ? 0.5 : 0.25));
			sendMovePosition();
			sendFaceDirection();
		}
		if (updateCount <= 0)
	...
	while (packet && tCont)
	{
		...
		if (tCont)mPeer->DeallocatePacket(packet);
...
void MagixNetworkManager::sendChat(const String &caption, const unsigned char &type, const String &pmTarget, bool autoLipSync)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendPartyChat(const OwnerToken &target, const String &caption, const unsigned char &type, bool autoLipSync)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processNewPlayer(Packet *p)
{
	...
	if (String(tMatName) == "CustomMat")
	{
		...
		tUnit->setMarkings((unsigned char)(mDef->hasBodyMark((unsigned char)tBodyMark) ? tBodyMark : 0), (unsigned char)(mDef->hasHeadMark((unsigned char)tHeadMark) ? tHeadMark : 0), (unsigned char)(mDef->hasTailMark((unsigned char)tTailMark) ? tTailMark : 0));
...
void MagixNetworkManager::processWorldInfo(Packet *p)
{
	...
	if (mUnitManager->getPlayer()->hasPet())
	{
		...
		const Vector3 tPosition = mUnitManager->getPlayer()->getPosition() + Vector3(Math::RangeRandom(-20, 20), tC.flying ? (Real)15 : (Real)0, -25);
...
void MagixNetworkManager::sendMapChange(const String &mapName, const unsigned char &dimension)
{
	sentPlayerUpdate = false;
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCreateAccount(const String &arg1, const String &arg2, const String &arg3, const String &arg4, const String &arg5)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendEditAccount(const String &arg1, const String &arg2, const String &arg3)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processLoadChar(Packet *p)
{
	...
	networkMessage = NetworkMessage((tSuccess ? NETWORK_LOADCHARSUCCESS : NETWORK_LOADCHARFAIL));
}
...
void MagixNetworkManager::processCreateChar(Packet *p)
{
	...
	tBitStream.Read(tID);
	tBitStream.Read(tSuccess);
	networkMessage = NetworkMessage((tSuccess ? NETWORK_CREATECHARSUCCESS : NETWORK_CREATECHARFAIL));
}
void MagixNetworkManager::sendCreateChar(const String &username, const String &name, const String &data)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processEditChar(Packet *p)
{
	...
	tBitStream.Read(tID);
	tBitStream.Read(tSuccess);
	networkMessage = NetworkMessage((tSuccess ? NETWORK_EDITCHARSUCCESS : NETWORK_EDITCHARFAIL));
}
void MagixNetworkManager::sendEditChar(const String &username, const short &charID, const String &data)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendKick(const String &name, bool isBan, const unsigned short numDays)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendItemEquip(const String &meshName, const unsigned short &slotID, const OwnerToken &target, bool dontTellMainServer)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendAcknowledge(const unsigned char &type, const unsigned short &iID)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterCreate(const String &type, const Vector3 &position, bool isPet)
{
...
void MagixNetworkManager::sendCritterCreate(const unsigned char &typeID, const Vector3 &position)
{
	const unsigned short tID = mCritterManager->getNextEmptyID((mUnitManager->getPlayer()->getIndex() - 1) * 200);
	mCritterManager->pushSentID(tID);
...
void MagixNetworkManager::sendCritterTakeover(const unsigned short &iID, const OwnerToken &token, bool fastestFirst)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterHit(const unsigned short &iID, const Real &damage, const Vector3 &force)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterAttackDecision(const unsigned short &iID, const unsigned short &targetID, const unsigned char &attackID, bool targetIsCritter)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::processCritterInfo(Packet *p)
{
	...
	if (tIsShrunk)
	{
		...
		if (tIsTrue)
		{
			...
			{
				const Real tScale = (Real)(tCritter->getLength()>50 ? 0.1 : (1 - tCritter->getLength()*0.018));
...
void MagixNetworkManager::sendCritterUpdate(const OwnerToken &target, const unsigned short &iID, const unsigned char &typeID, const Vector3 &position, const Real &hpRatio, const OwnerToken &token)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterUpdate(const OwnerToken &target, const unsigned short &iID, const String &type, const Vector3 &position, const Real &hpRatio, bool isPet, const OwnerToken &token)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterUpdateRequest(const OwnerToken &target, const unsigned short &iID)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendCritterShrinkTargeted(const OwnerToken &target, const unsigned short &iID, bool isEvolve)
{
	RakNet::BitStream tBitStream;
...
void MagixNetworkManager::sendPlayerHit(const OwnerToken &unitID, const Real &damage, const Vector3 &force)
{
	RakNet::BitStream tBitStream;
